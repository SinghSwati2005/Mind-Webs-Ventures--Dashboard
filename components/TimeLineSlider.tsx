...
export default function TimeLineSlider() {
  ...
  // 2️⃣ Update localValue on unit or range toggle
  useEffect(() => {
    if (rangeType === "range") {
      const start = 0;
  ...
  // 3️⃣ Set metrics and update zustand dashboard store
  useEffect(() => {
    if (rangeType === "range") {
    ...
    const delta = rangeType === "range" ? localValue[1] - localValue[0] : localValue[0];
  ...
  // ✅ 4️⃣ NEW: Update useTimelineStore (moved inside component)
useEffect(() => {
  const getDateFromIndex = (index: number, unit: "day" | "month" | "year") => {
    if (typeof index !== "number" || isNaN(index)) return null;

    if (unit === "day") return addDays(START_DATE, index * 7);
    if (unit === "month") return addMonths(START_DATE, index);
    if (unit === "year") return addYears(START_DATE, index);
    return null;
  };

  if (localValue.length === 0) return;

  if (rangeType === "range" && localValue.length === 2) {
    const [startIndex, endIndex] = localValue;

    const startDateObj = getDateFromIndex(startIndex, timeUnit);
    const endDateObj = getDateFromIndex(endIndex, timeUnit);
    ...
    useTimelineStore.getState().setSelectedRange({ start: startDate, end: endDate });
  }

  if (rangeType === "single" && localValue.length === 1) {
    const pointIndex = localValue[0];
    const singleDateObj = getDateFromIndex(pointIndex, timeUnit);

    if (!singleDateObj) return;

    const singleDate = format(singleDateObj, "yyyy-MM-dd");

    useTimelineStore.getState().setSelectedRange({ start: singleDate, end: singleDate });
  }
}, [localValue, rangeType, timeUnit]);
          ...
          <div className="flex items-center gap-2 text-sm">
            ...
            <label className="relative inline-flex items-center cursor-pointer">
              <input
                ...
                onChange={() => setRangeType(rangeType === "range" ? "single" : "range")}
